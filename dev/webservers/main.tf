...
data "terraform_remote_state" "public_caleb" { // This is to use Outputs from Remote State
  backend = "s3"
  config = {
...
resource "aws_instance" "acs73026" {
  ...
  count                       = length(data.terraform_remote_state.public_caleb.outputs.private_subnet_ids)
  instance_type               = lookup(var.instance_type, var.env)
  key_name                    = aws_key_pair.assignment1.key_name
  security_groups             = [aws_security_group.acs730w7.id]
  subnet_id                   = data.terraform_remote_state.public_caleb.outputs.private_subnet_ids[count.index]
...
resource "aws_instance" "bastion_acs73026" {

  ami                         = data.aws_ami.latest_amazon_linux.id
  instance_type               = lookup(var.instance_type, var.env)
  key_name                    = aws_key_pair.assignment1.key_name
  security_groups             = [aws_security_group.acs730w7.id]
  subnet_id                   = data.terraform_remote_state.public_caleb.outputs.public_subnet_ids[1]
  associate_public_ip_address = true
  
  tags = merge(local.default_tags,
    {
      "Name" = "${var.prefix}bastion-Amazon-Linux"
    }
  )
...
resource "aws_security_group" "acs730w7" {
  ...
  vpc_id      = data.terraform_remote_state.public_caleb.outputs.vpc_id
